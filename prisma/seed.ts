  ...
  for (let i = 0; i < 20; i++) {
    await prisma.user.create({
      data: {
        ...
        mpesaReferenceId: faker.datatype.uuid(),
        companyName: faker.company.name(),
        companySize: `${faker.datatype.number({ min: 1, max: 10 })}-${faker.datatype.number({ min: 11, max: 50 })} employees`,
        companyLocation: faker.address.country(),
